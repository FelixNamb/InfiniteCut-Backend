...
router.post('/signup', (req, res) => {
    ...
    if (!checkBody(req.body, ['prenom', 'nom', 'codePostal', 'mobile', 'email'])){
        res.json({ result: false, error: 'Missing or empty fields' });
        return;
    };

    UserPro.findOne({codePostal, mobile, email})
    .then(data => {
        if(!data){
            ...
            res.json({result: false, message: "Ce compte existe déjà. Voulez vous vous connecter ?"});
        }
    });
});
...
router.put('/', (req, res) => {
    UserPro.updateOne({token: req.body.token}, {$addToSet : {formules: req.body._ObjectId}})
    .then(()=>{
        UserPro.findOne({token: req.body.token}).populate('formules').then(data => {
